...
public class CommunityModelImpl implements CommunityModel {
    ...
    @Override
    public void getHis(int uId, int writerId, final CallBackListener listener) {
        api.getHis(uId, writerId).subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Action1<ArticleBean>() {
                    ...
                    public void call(ArticleBean articleBean) {
                        if(articleBean.getCode() == Constants.SUCCESS){
                            ...
                            listener.onError(new Exception("出错了"));
                        }
                    }
                });
    }
